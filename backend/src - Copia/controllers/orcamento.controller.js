...
const atualizar = async (req, res, next) => {
  try {
    res.json(await svc.atualizar(+req.params.id, req.body, req.usuario.id));
  } catch (e) {
    next(e);
  }
};
...
const definirFornecedorOculto = async (req, res, next) => {
  try {
    ...
    res.json(await svc.definirFornecedorOculto(+req.params.id, +fornecedorId, oculto, req.usuario.id));
  } catch (e) {
    next(e);
  }
};
...
const streamSSE = async (req, res) => {
  ...
  svc.adiarLiberacaoPorDesconexao(userId);

  req.on('close', () => {
    clearInterval(keepAlive);
    svc.orcamentoEvents.removeListener('evento', listener);
    svc.agendarLiberacaoPorDesconexao(userId);
  });
};
