...
const listarTodas = async (req, res, next) => {
  try {
    const q = { ...req.query };
    q.apenasNumericas = q.apenasNumericas === 'true';
    q.apenasTextuais  = q.apenasTextuais === 'true';
    res.json(await svc.listarTodas(q));
  }
  catch(e){next(e);}
};

const listarMovimentacoes = async (req, res, next) => {
  try { res.json(await svc.listarMovimentacoes(req.query)); }
  catch(e){ next(e); }
};
...
module.exports = { listar, buscarPorNumeroOS, buscarClientePorNumeroOS, registrarMovimentacao, removerMovimentacao, excluirRelacaoOS, fecharOS, listarTodas, listarMovimentacoes, renomearOS, atualizarPrecoMovimentacao };