...
// Fontes com suporte a acentuação (á, ç, ã...) e símbolos (×, ø, ½, ¼, –, ² etc).
// As fontes padrão do PDFKit (Helvetica) usam a tabela WinAnsi e não possuem
// glifos para vários desses caracteres, o que corrompia os nomes dos materiais no PDF.
const FONT_REGULAR = path.join(__dirname, '../assets/fonts/DejaVuSans.ttf');
const FONT_BOLD    = path.join(__dirname, '../assets/fonts/DejaVuSans-Bold.ttf');

// Sanitiza texto vindo do banco/frontend antes de desenhar no PDF.
// Corrige tabs, quebras de linha soltas e outros caracteres de controle
// que não têm glifo em nenhuma fonte e aparecem como "□" no PDF.
function limparTexto(valor) {
  if (valor == null) return valor;
  return String(valor)
    // tabs e quebras de linha -> espaço simples
    .replace(/[\t\r\n\v\f]+/g, ' ')
    // demais caracteres de controle (exceto espaço normal) -> remove
    // eslint-disable-next-line no-control-regex
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, '')
    // espaços duplicados -> um só
    .replace(/ {2,}/g, ' ')
    .trim();
}

// Trunca um texto com reticências para caber em uma única linha dentro de
// maxWidth, usando a fonte/tamanho atualmente selecionados no doc.
// Evita que nomes longos (ex: razão social de fornecedor) quebrem em
// múltiplas linhas e sobreponham o conteúdo abaixo em células de altura fixa.
...
async function gerarPdfDeItens(dados) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Altura do cabeçalho é calculada dinamicamente: medimos quantas linhas
    // o nome de cada fornecedor precisa (sem truncar) e usamos a maior altura
    // necessária, para que nomes longos nunca sejam cortados nem sobreponham
    // o conteúdo abaixo.
    const HDR_PAD_TOP    = 7;
