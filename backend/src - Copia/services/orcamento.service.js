...
// Fila de execução por orçamento: garante que escritas concorrentes de
// itens (ex.: autosave debounced + edição manual de preço) para o MESMO
// orçamento sejam aplicadas em ordem de chegada, uma de cada vez — nunca
// em paralelo. Sem isso, uma requisição mais lenta pode terminar depois
// de uma mais recente e sobrescrever silenciosamente dados mais novos
// (o "preço salva mas some depois de atualizar" relatado pelos usuários).
...
async function atualizarItem(itemId, data, origemUsuarioId = null) {
  // Precisamos saber o orcamentoId antes de enfileirar, para serializar
  // por orçamento (e não globalmente).
