    ...
    materiais: item.materiais.map((m) => {
      ...
      return {
        ...
        material: {
          ...
          // Dimensões de referência da chapa (usadas para cálculo de custo/m²)
...
function _serializarOrcamento(ov) {
  return {
    id:          ov.id,
    numero:      ov.numero,
    status:      ov.status,
    ...
    cliente: ov.cliente
      ? { id: ov.cliente.id, nome: ov.cliente.nome }
      : null,
    criador: ov.criador
      ? { id: ov.criador.id, nome: ov.criador.nome }
      : null,
...
async function listar(filtros = {}) {
  ...
  return lista.map(_serializarOrcamento);
}
...
async function buscarPorId(id) {
  ...
  return _serializarOrcamento(ov);
}
 ...
 * Body esperado:
 * {
 *   clienteNome: string,          // nome livre (armazenado no cliente ou número)
 *   numero?: string,              // número personalizado; auto-gerado se omitido
 *   observacao?: string,
 *   margemLucro?: number,
 *   criadorId?: number,
 *   clienteId?: number,
...
async function criar(data) {
  ...
  const ov = await prisma.orcamentoVenda.create({
    data: {
      ...
      // Cria os itens (produtos + materiais) junto com o orçamento
      itens: itens.length > 0 ? { create: itens.map(_buildItemCreate) } : undefined,
    },
    include: _include,
  });

  // Calcula e persiste valorTotal
  const valorTotal = _calcularTotal(ov);
  const atualizado = await prisma.orcamentoVenda.update({
    where:   { id: ov.id },
    data:    { valorTotal },
    include: _include,
  });
  return _serializarOrcamento(atualizado);
}
...
async function atualizar(id, data) {
  ...
  const valorTotal = _calcularTotal(atualizado);
  const final_ = await prisma.orcamentoVenda.update({
    where:   { id },
    data:    { valorTotal },
    include: _include,
  });
  return _serializarOrcamento(final_);
}
...
async function _recalcularTotal(orcamentoVendaId) {
  const ov = await prisma.orcamentoVenda.findUnique({
    ...
    include: { itens: { include: { materiais: true } } },
  });
  if (!ov) return;
  const valorTotal = _calcularTotal(ov);
