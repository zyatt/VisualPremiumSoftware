...
const excluirRelacaoOS    = async (req, res, next) => { try { await svc.excluirRelacaoOS(req.params.numeroOS); res.status(204).send(); } catch(e){next(e);} };
const fecharOS            = async (req, res, next) => { try { res.json(await svc.fecharOS(req.params.numeroOS)); } catch(e){next(e);} };

module.exports = { listar, buscarPorNumeroOS, registrarMovimentacao, removerMovimentacao, excluirRelacaoOS, fecharOS };