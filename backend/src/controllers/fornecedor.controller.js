...
const remover     = async (req,res,next) => { try { res.json(await svc.remover(+req.params.id)); res.status(204).send(); } catch(e){next(e);} };

const vincularMaterial = async (req,res,next) => {
  try {
    const { materialId, preco, precoMetroQuadrado, prazoEntrega } = req.body;
    res.json(await svc.vincularMaterial(+req.params.id, materialId, preco, precoMetroQuadrado, prazoEntrega));
  } catch(e){next(e);}
};

const desvincularMaterial = async (req,res,next) => {
  try {
    res.json(await svc.desvincularMaterial(+req.params.id, +req.params.materialId));
  } catch(e){next(e);}
};

const atualizarPreco = async (req,res,next) => {
  try {
    res.json(await svc.atualizarPrecoVinculo(+req.params.id, +req.params.materialId, req.body));
  } catch(e){next(e);}
};

module.exports = { listar, buscarPorId, criar, atualizar, remover, vincularMaterial, desvincularMaterial, atualizarPreco };